...
// Synthesize pleasant chime using Web Audio API (no external asset needed)
function playChime() {
  try {
...
export default function Demo() {
  ...
  const jumpToNextBag = () => {
    ...
    if (next) {
      ...
      // Loop back to first bag
      jumpToBag(BAG_LOADING_EVENTS[0]);
    }
  };
          ...
          <div className="flex items-center gap-4">
            <nav className="hidden sm:flex items-center gap-1 text-sm">
              <button
                ...
                className="px-3 py-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 font-medium transition"
              >
                Monitoring
              </button>
              <button
                onClick={() => navigate('/demo')}
                className="px-3 py-2 rounded-lg bg-amber-500/20 text-amber-300 font-semibold border border-amber-500/30 transition"
              >
                Accurate Demo
              </button>
              <button
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 py-6 flex flex-col gap-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-950/50 p-4 rounded-xl border border-slate-800">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-xl sm:text-2xl font-black tracking-tight text-white">
                Live Video Bag Counting Verification
              </h1>
              <span className="bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-2.5 py-0.5 rounded-full text-xs font-medium">
                100% Video Accurate
              </span>
            </div>
            <p className="text-slate-400 text-sm mt-1">
              Feed: <span className="text-slate-200 font-medium">{DEMO_VIDEO.bay}</span> ·
              Target Vehicle: <span className="text-amber-400 font-semibold">{DEMO_VIDEO.truckId}</span> ·
              Model: <span className="text-slate-300">YOLO-11x ChuteZone</span>
            </p>
          </div>
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            <button
              onClick={jumpToNextBag}
              className="px-3.5 py-2 rounded-lg bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-sm transition shadow-lg shadow-amber-500/20 flex items-center gap-2"
            >
            ...
            <button
              ...
              className="px-3 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white font-medium text-sm transition border border-slate-700"
            >
              Reset 0:00
            </button>
            <button
              onClick={() => setSoundAlerts(!soundAlerts)}
              className={`px-3 py-2 rounded-lg font-medium text-sm transition border flex items-center gap-1.5 ${
                soundAlerts
                  ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40'
                  : 'bg-slate-800 text-slate-400 border-slate-700 hover:text-slate-200'
              }`}
              title="Toggle audio chime on bag count"
            >
              <span>{soundAlerts ? '🔊 Chime On' : '🔇 Chime Off'}</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-8 flex flex-col gap-3">
            <div className="relative aspect-video bg-black rounded-2xl overflow-hidden border border-slate-800 shadow-2xl group">
              ...
              {/* Toast banner when a bag is loaded */}
              {justLoadedId && (
                <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 bg-emerald-500/95 text-slate-950 px-5 py-2 rounded-full font-extrabold text-sm shadow-xl flex items-center gap-2.5 animate-bounce">
              ...
              {activeBagInTransit && !justLoadedId && (
                <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 bg-amber-500/90 text-slate-950 px-4 py-1.5 rounded-full font-bold text-xs tracking-wide shadow-lg flex items-center gap-2 animate-pulse">
              ...
              <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-slate-950 via-slate-950/80 to-transparent p-4 flex flex-col gap-2 opacity-95 group-hover:opacity-100 transition">
                {/* Custom Scrubber Bar with Bag Markers */}
                <div className="relative w-full h-4 flex items-center cursor-pointer group/bar">
                  <input
                    type="range"
                    min={0}
                    max={duration || DEMO_VIDEO.duration}
                    step={0.1}
                    value={currentTime}
                  ...
                  {BAG_LOADING_EVENTS.map((bag) => {
                    ...
                    return (
                      <button
                        ...
                        style={{ left: `${dotPct}%` }}
                        className={`absolute -translate-x-1/2 z-20 w-3 h-3 rounded-full border-2 transition hover:scale-150 ${
                          isPassed
                            ? 'bg-emerald-400 border-slate-900 shadow-md shadow-emerald-500/50'
                ...
                <div className="flex items-center justify-between text-xs text-slate-300">
                  <div className="flex items-center gap-3">
                    <button
                      onClick={togglePlay}
                      className="p-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-white font-semibold transition"
                    >
                      {isPlaying ? (
                        <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current">
                    ...
                    <span className="font-mono text-slate-200">
                      {formatTime(currentTime)} / {formatTime(duration || DEMO_VIDEO.duration)}
                    </span>
                  </div>
          ...
          {/* Real-time KPI & Audit Feed Column (5 Cols) */}
          <div className="lg:col-span-4 flex flex-col gap-4">
            {/* Primary Hero Bag Counter Card */}
            <div className="p-6 rounded-2xl bg-gradient-to-br from-slate-950 to-slate-900 border border-slate-800 shadow-xl relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-2">
                ...
                <span className="text-xs text-slate-500 font-mono">
                  {TOTAL_BAGS_IN_SESSION} total in clip
                </span>
              </div>
              ...
              <div className="flex items-baseline gap-3 my-2">
                ...
                <span className="text-2xl font-bold text-slate-500">
                  / {TOTAL_BAGS_IN_SESSION}
                </span>
                <span className="ml-auto text-sm font-semibold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-full">
                  {targetPct}%
                </span>
              </div>

              <p className="text-xs text-slate-400 mt-1">
                Cement Bags loaded through Bay 1 Chute into {DEMO_VIDEO.truckId}
              </p>
              ...
              <div className="grid grid-cols-2 gap-3 mt-5 pt-4 border-t border-slate-800/80">
                <div className="bg-slate-900/60 p-3 rounded-xl border border-slate-800/50">
                  ...
                  <p className="text-lg font-bold text-white mt-0.5">
                    {(count * 50).toLocaleString()} <span className="text-xs text-slate-400 font-normal">kg</span>
                  </p>
                </div>
                <div className="bg-slate-900/60 p-3 rounded-xl border border-slate-800/50">
                ...
                {loadedBags.length === 0 ? (
                  ...
                  [...loadedBags].reverse().map((bag) => (
                    <div
                      key={bag.id}
                      onClick={() => jumpToBag(bag)}
                      className={`pt-2.5 pb-1 px-3 rounded-lg cursor-pointer transition flex items-center justify-between hover:bg-slate-900 ${
                        bag.id === justLoadedId ? 'bg-emerald-950/40 border border-emerald-500/40' : ''
                      }`}
                    >
                      <div className="flex items-center gap-3">
